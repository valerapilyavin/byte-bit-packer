...
export async function tst10() {
    it('forAll', async (ctx) => {
        ...
        await ctx.test("forAllUInt64", () => {
            ...
            packerTrue.writeArrU64(0, [fromBigInt(1000000000000n), fromBigInt(1000000000500n)]);
            packerFalse.writeArrU64(0, [fromBigInt(1000000000000n), fromBigInt(1000000000500n)], true);
            let counter = 0;
            function forAll_1(index, data) {
                if (index == 0) {
                    if (toBigInt(data) != 1000000000000n) {
                        throw new Error("forAll_1_True:data != 1_000_000_000_000n");
                    }
                }
                else if (index == 1) {
                    if (toBigInt(data) != 1000000000500n) {
        ...
        await ctx.test("forAllInt64", () => {
            let packerTrue = new BBPacker(17, { littleEndian: true });
            let packerFalse = new BBPacker(17, { littleEndian: false });
            packerTrue.writeArrI64(0, [fromBigInt(-1000000000000n), fromBigInt(-1000000000500n)]);
            packerFalse.writeArrI64(0, [fromBigInt(-1000000000000n), fromBigInt(-1000000000500n)], true);
            let counter = 0;
            function forAll_1(index, data) {
                if (index == 0) {
                    if (toBigInt(data) != -1000000000000n) {
